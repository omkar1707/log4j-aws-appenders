...
implements WriterFactory<SNSWriterConfig,SNSWriterStatistics>
{
    ...
    public LogWriter newLogWriter(SNSWriterConfig config, SNSWriterStatistics stats, InternalLogger logger)
    {
        return new SNSLogWriter(
                config, stats, logger,
                new DefaultClientFactory<AmazonSNS>(AmazonSNS.class, config.clientFactoryMethod, config.clientRegion, config.clientEndpoint, logger, config.clientAccessKey, config.clientSecretKey));
    }
}
